    ...
    <div x-show="!loading" x-transition.opacity>
        ...
        <div x-show="detalleModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4" style="display: none;">
            <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl relative border border-gray-100">
                ...
                <!-- Banner de Cancelación/Devolución si aplica (Solo para cancelados o devueltos) -->
                <template x-if="['CANCELADO', 'NO_ENTREGADO', 'cancelado', 'no_entregado'].includes(selectedPedido?.estado || '') || ['CANCELADO', 'NO_ENTREGADO', 'cancelado', 'no_entregado'].includes(detalleFull?.estado || '')">
                    <div class="mb-5 bg-rose-50 border border-rose-200 rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div>
                            <h4 class="font-extrabold text-xs text-rose-900 uppercase tracking-wider" x-text="(selectedPedido?.estado || '').includes('NO_ENTREGADO') ? 'Devolución de Pedido' : 'Pedido Cancelado'"></h4>
                            <p class="text-xs text-rose-700 font-medium mt-0.5" x-text="detalleFull?.motivo_cancelacion || 'Este pedido ha sido cancelado o devuelto.'"></p>
                        </div>
                    </div>
                </template>
                            ...
                            <div class="bg-rose-50/40 rounded-xl border border-rose-200 overflow-hidden shadow-2xs">
                                <table class="w-full text-left text-xs">
                                    ...
                                    <tbody class="divide-y divide-rose-100">
                                        <template x-for="item in (detalleFull?.items || []).filter(i => (i.cantidad_solicitada - (i.cantidad_entregada || 0)) > 0)" :key="'dev_' + item.id">
                                            <tr class="hover:bg-rose-50/80">
                                                ...
                                                <td class="py-3 px-4 text-right text-gray-600 font-medium" x-text="`$${Number(item.precio_unitario).toFixed(2)}`"></td>
                                                <td class="py-3 px-4 text-right font-black text-rose-700" x-text="`$${((item.cantidad_solicitada - (item.cantidad_entregada || 0)) * Number(item.precio_unitario) * 1.15).toFixed(2)}`"></td>
                                                <td class="py-3 px-4">
                                                    <span class="inline-block bg-rose-100 border border-rose-200 text-rose-800 px-2 py-0.5 rounded text-[10px] font-extrabold"
                                                          x-text="item.motivo_devolucion || detalleFull.motivo_cancelacion || 'Otro motivo'"></span>
                                                </td>
                                            </tr>
                                        </template>
                                    </tbody>
